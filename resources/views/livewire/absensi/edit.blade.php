                ...
                <div class="p-6 space-y-6">
                        ...
                        <div class="flex items-start">
                            ...
                            <div class="ml-3">
                                ...
                                <div class="mt-2 text-sm text-blue-700 grid grid-cols-1 sm:grid-cols-2 gap-2">
                                    ...
                                    <p><strong>Tanggal:</strong> {{ $absensi->tanggal->locale('id')->translatedFormat('d M Y') }}</p>
                                    <p><strong>Status:</strong> {{ ucfirst($absensi->status) }}</p>
                                    <p><strong>Dicatat:</strong> {{ $absensi->created_at->locale('id')->translatedFormat('d M Y H:i') }}</p>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <div class="grid grid-cols-2 gap-4 sm:grid-cols-4">
                            {{-- Hadir --}}
                            <label
                                class="relative flex items-center justify-center p-4 border rounded-lg cursor-pointer transition-all duration-200 
                                   @if ($status === 'hadir') border-green-500 bg-green-50 ring-2 ring-green-200 
                                   @else 
                                       border-gray-300 hover:border-green-300 hover:bg-green-25 @endif">
                                <input type="radio" wire:model.live="status" value="hadir" class="sr-only">
                            ...
                            <label
                                class="relative flex items-center justify-center p-4 border rounded-lg cursor-pointer transition-all duration-200 
                                   @if ($status === 'alpha') border-red-500 bg-red-50 ring-2 ring-red-200 
                                   @else 
                                       border-gray-300 hover:border-red-300 hover:bg-red-25 @endif">
                                <input type="radio" wire:model.live="status" value="alpha" class="sr-only">
                                <div class="text-center">
                                    <div class="flex justify-center mb-2">
                                        <div
                                            class="w-8 h-8 rounded-full transition-all duration-200 
                            ...
                            <label
                                class="relative flex items-center justify-center p-4 border rounded-lg cursor-pointer transition-all duration-200 
                                   @if ($status === 'sakit') border-yellow-500 bg-yellow-50 ring-2 ring-yellow-200 
                                   @else 
                                       border-gray-300 hover:border-yellow-300 hover:bg-yellow-25 @endif">
                                <input type="radio" wire:model.live="status" value="sakit" class="sr-only">
                                <div class="text-center">
                                    <div class="flex justify-center mb-2">
                                        <div
                                            class="w-8 h-8 rounded-full transition-all duration-200 
                            ...
                            <label
                                class="relative flex items-center justify-center p-4 border rounded-lg cursor-pointer transition-all duration-200 
                                   @if ($status === 'izin') border-blue-500 bg-blue-50 ring-2 ring-blue-200 
                                   @else 
                                       border-gray-300 hover:border-blue-300 hover:bg-blue-25 @endif">
                                <input type="radio" wire:model.live="status" value="izin" class="sr-only">
                                <div class="text-center">
                                    <div class="flex justify-center mb-2">
                                        <div
                                            class="w-8 h-8 rounded-full transition-all duration-200 
                        ...
                        <div class="bg-gray-50 border border-gray-200 rounded-lg p-4">
                            ...
                            <div class="text-sm text-gray-700">
                                @switch($status)
                                    ...
                                    @case('sakit')
                                        <p>🏥 <strong>Sakit:</strong> Pegawai tidak hadir karena sakit (dengan/tanpa surat
                                            dokter).</p>
                                    @break

                                    @case('izin')
                                        <p>📝 <strong>Izin:</strong> Pegawai tidak hadir dengan izin atau keperluan tertentu.
                                        </p>
                                    @break
                                @endswitch
                            </div>
                ...
                <dl class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                    ...
                    <div>
                        ...
                        <dd class="mt-1 text-sm text-gray-900">
                            {{ $absensi->created_at->locale('id')->translatedFormat('d F Y H:i') }}</dd>
                    </div>
                    <div>
                        <dt class="text-sm font-medium text-gray-500">Terakhir Diupdate</dt>
                        <dd class="mt-1 text-sm text-gray-900">
                            {{ $absensi->updated_at->locale('id')->translatedFormat('d F Y H:i') }}</dd>
                    </div>
                    <div>
